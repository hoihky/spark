...
const UiTheme& UiPaintContext::GetTheme() const noexcept {
    static const UiTheme kFallback = UiTheme::ClassicMint();
    return themePtr != nullptr ? *themePtr : kFallback;
}

const UiLayoutMetrics& UiPaintContext::GetLayoutMetrics() const noexcept {
    static const UiLayoutMetrics kFallback = UiLayoutMetrics::Default();
    return metricsPtr != nullptr ? *metricsPtr : kFallback;
}

float UiPaintContext::MeasureUtf8Width(const Utf8String& text, const float sizePixels) const noexcept {
    return MeasureWidth(layoutFont, text, sizePixels, 0.52F);
}

float UiPaintContext::GetLineSpacingPixels(const float sizePixels) const noexcept {
    return layoutFont != nullptr ? layoutFont->GetLineSpacingPixels(sizePixels) : sizePixels * 1.2F;
}

void UiPaintContext::BuildWrappedLines(
        const Utf8String& text,
...
Utf8String UiPaintContext::EllipsizeUtf8(
        const Utf8String& text,
        const float sizePixels,
...
void UiPaintContext::DrawTextInRect(
        const Rect& rect,
        const float sizePixels,
        const Utf8String& text,
        const Vector3& rgb,
...
bool UiPaintContext::CurrentClipAllowsDraw() const noexcept {
...
bool UiPaintContext::ClipAllowsImmediateDraw() const noexcept {
...
void UiPaintContext::AttachClipTo(ScreenRectDraw& d) const noexcept {
    if (clipStack.IsEmpty()) {
        d.clipEnabled = false;
        return;
    }
...
void UiPaintContext::AttachClipTo(ScreenSpriteDraw& d) const noexcept {
    if (clipStack.IsEmpty()) {
        d.clipEnabled = false;
        return;
    }
...
void UiPaintContext::AttachClipTo(ScreenTextDraw& d) const noexcept {
    if (clipStack.IsEmpty()) {
        d.clipEnabled = false;
        return;
    }
...
void UiPaintContext::PushClipRect(const float x, const float y, const float width, const float height) {
...
void UiPaintContext::PopClipRect() noexcept {
    if (!clipStack.IsEmpty()) {
        clipStack.PopBack();
    }
}

void UiPaintContext::PopOverlayLayer() noexcept {
    if (overlayDepth > 0) {
        --overlayDepth;
    }
}

void UiPaintContext::PopLateLayer() noexcept {
    if (lateDepth > 0) {
        --lateDepth;
    }
}

void UiPaintContext::FillRoundRectSolid(
...
void UiPaintContext::FillRoundRectGradientVertical(
        float x,
        float y,
        float w,
        float h,
...
void UiPaintContext::StrokeRoundRect(
        float x,
        float y,
        float w,
        float h,
...
void UiPaintContext::FillRectGradientVertical(
        float x,
        float y,
        float w,
        float h,
...
void UiPaintContext::FillRectGradientHorizontal(
        float x,
        float y,
        float w,
        float h,
...
void UiPaintContext::FillDropShadow(
        float x,
        float y,
        float w,
        float h,
...
void UiPaintContext::StrokeRect(
...
void UiPaintContext::DrawTextWrapped(
        const float x,
        const float y,
        const float maxWidth,
        const float sizePixels,
...
void UiPaintContext::SetFramebufferPixelSize(float width, float height) noexcept {
    fbW = width > 0.0F ? width : 1.0F;
    fbH = height > 0.0F ? height : 1.0F;
    guiSpriteSortCounter = 50000;
}

void UiPaintContext::EmitSprite(
...
std::int32_t UiPaintContext::AcquireUiTextureLayer(const SharedPtr<Texture2D>& texture) {
...
void UiPaintContext::DrawSpriteSlice(
        const UiSpriteSlice& slice,
        const float x,
        const float y,
        const float w,
        const float h,
...
void UiPaintContext::DrawNineSlice(
        const UiSpriteSlice& slice,
        const float x,
        const float y,
        const float w,
