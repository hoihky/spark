...
void VulkanGpuMeshInterleaved::AppendRigidMeshVertexToInterleaved(const Mesh::Vertex& v, Array<float>& interleaved) {
    ...
    interleaved.PushBack(v.texCoord1.x);
    interleaved.PushBack(v.texCoord1.y);
    ...
    interleaved.PushBack(v.color.x);
    interleaved.PushBack(v.color.y);
    interleaved.PushBack(v.color.z);
    interleaved.PushBack(v.color.w);
