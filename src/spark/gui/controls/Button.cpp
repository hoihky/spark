...
void Button::Paint(GuiPaintContext& ctx) const {
    ...
    if (preferSkinChrome && !accentSelected) {
        const GuiSkin* skin = ctx.GetSkin();
        GuiSpriteSlice skinSlice{};
        GuiSkinElement skinSlot = GuiSkinElement::ButtonNormal;
        if (!enabled) {
            skinSlot = GuiSkinElement::ButtonDisabled;
        } else if (ctx.IsActive(this)) {
            skinSlot = GuiSkinElement::ButtonPressed;
        } else if (ctx.IsHot(this)) {
            skinSlot = GuiSkinElement::ButtonHover;
        }
        if (skin != nullptr && skin->TryGetSlice(skinSlot, skinSlice) && skinSlice.IsValid()) {
            ctx.DrawNineSlice(skinSlice, bounds.x, bounds.y, bounds.width, bounds.height);
            paintedSkin = true;
            textRgb = t.labelPrimary;
        }
    }
    ...
    if (!iconGlyph.IsEmpty()) {
        ...
        const float isz = std::min(drawIcon, std::max(m.Scaled(10.0F), bounds.height - m.Scaled(6.0F)));
        const float iy = bounds.y + std::max(m.Scaled(2.0F), (bounds.height - isz) * 0.5F);
        GuiSpriteSlice iconSlice{};
        iconSlice.texture = iconTexture;
        iconSlice.uvRect = {0.0F, 0.0F, 1.0F, 1.0F};
        ctx.DrawSpriteSlice(iconSlice, bounds.x + padInner, iy, isz, isz);
        textX = bounds.x + padInner + isz + m.Scaled(6.0F);
        maxTextW = bounds.x + bounds.width - padX - textX;
    }
