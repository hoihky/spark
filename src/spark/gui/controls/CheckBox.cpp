...
void CheckBox::Paint(GuiPaintContext& ctx) const {
    ...
    if (preferSkinChrome) {
        ...
        GuiSpriteSlice boxSlice{};
        const GuiSkinElement slot = checked ? GuiSkinElement::CheckboxOn : GuiSkinElement::CheckboxOff;
        if (skin != nullptr && skin->TryGetSlice(slot, boxSlice) && boxSlice.IsValid()) {
            ctx.DrawSpriteSlice(boxSlice, bounds.x, boxTop, box, box);
            paintedSkin = true;
        }
    }
