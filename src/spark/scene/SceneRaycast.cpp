...
[[nodiscard]] bool TryRaycastSkinnedMeshWorld(
...
bool Scene::RaycastPick(const SceneRay& ray, SceneRaycastHit& outHit, const SceneRaycastOptions& options) const {
    outHit = SceneRaycastHit{};
    if (ray.direction.LengthSquared() < 1.0e-12F) {
        return false;
    }

    const Vector3 rd = ray.direction.Normalized();
    ...
    if (options.pickSkinnedMeshes) {
        visitObjects([&](GameObject* object) {
            ...
            const SkinnedMeshComponent* skinned = object->GetComponent<SkinnedMeshComponent>();
            if (skinned == nullptr || !skinned->GetMesh()) {
                return;
            }
            const Matrix4 worldMatrix = object->GetWorldMatrix();
            float t = 0.0F;
            if (!TryRaycastSkinnedMeshWorld(
                        ray.origin, rd, *skinned->GetMesh(), worldMatrix, ray.tMin, bestT, t)) {
                return;
            }
            bestT = t;
            bestObject = object;
    ...
    outHit.pointWorld = {
            ray.origin.x + rd.x * bestT,
            ray.origin.y + rd.y * bestT,
            ray.origin.z + rd.z * bestT};
    return true;
}
