...
| **Toggle & tuning** | `SceneRenderParams::directionalShadowsEnabled`, `shadowCascadeNear` / `shadowCascadeFar` (0 = preset), `shadowBias`, `shadowNormalBias`, `shadowsCastByDefault`, `shadowsReceiveByDefault`. |
| **CSM edge robustness** | `scene.frag` / `water_sun_shadow.glsl`: cascade-local UV outside `[0,1]` or clip Z outside `[0,1]` → treated as lit; `VulkanDirectionalShadowCascadeMath` expands ortho padding for large horizontal footprints. |
...
| **Time-of-day demo** | SparkDemo menu **17** / key **N** — `TimeOfDayDemo` animates `timeOfDay` 0→1 over 90s (sunshine → sunset → night → dawn). Props: Khronos **`Lantern.glb`** on a ground plane with HDR sky dome, fog/post volumes, and SSAO. Submit uses `FillStandardLitSceneFromWorld`. |
...
| **Per-draw shadow flags** | `SceneDrawItem::shadowFlags` (`kSceneShadowCast` / `kSceneShadowReceive`) — shadow pass skips non-casters; lit pass skips receivers. Resolved in scene submit via `ResolveDrawableShadowFlags` (terrain always casts unless overridden; submerged `GroundPlane` / low `UnitCube` props do not cast). |
| **`MaterialComponent` shadow overrides** | `SetShadowCastOverride` / `SetShadowReceiveOverride` — per-draw opt-out without changing global `shadowsCastByDefault`. |
...
| P0 | **Image-based ambient** | **Partial:** equirect IBL for PBR metals (sky dome + `ibl.glsl` + **BRDF LUT**); no SH probes or irradiance volumes yet. |
...
5. **Water (optional)** — when `waterDraws` is non-empty: `VulkanWaterPass` records after the snapshot resume using `water.vert` / `water.frag` (Gerstner displacement, crest foam via Jacobian threshold, layered procedural detail normals, screen-space refraction of binding **13**, Beer–Lambert depth absorption from binding **14**, Fresnel sky IBL + SSR, sun GGX specular with CSM via `water_sun_shadow.glsl`). Water draws are sorted back-to-front via `SceneRenderParams::waterSortMode` (default `BackToFrontByDepth`). **Depth test is disabled** on the water pipeline; fragments above opaque geometry are discarded in `water.frag` using binding **14**. Water **never casts** (`kSceneShadowCast` off); **receives** directional CSM when `directionalShadowsEnabled` and `ResolveWaterShadowFlags` resolves receive (material override respected).
6. **Transparent (optional)** — when `transparentDraws` is non-empty: `VulkanSceneOpaquePass::RecordTransparent` **after all water draws** (same resumed HDR pass); sorted via `SceneRenderParams::transparentSortMode`. glTF transmission samples binding **13** (`sceneOpaqueColor`). Water always renders before other transparent meshes so glass/transmission composites on top.
