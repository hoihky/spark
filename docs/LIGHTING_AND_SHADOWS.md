...
5. **Water (optional)** — when `waterDraws` is non-empty: `VulkanWaterPass` records after the snapshot resume using `water.vert` / `water.frag` (Gerstner displacement, crest foam via Jacobian threshold, layered procedural detail normals, screen-space refraction of binding **13**, Beer–Lambert depth absorption from binding **14**, Fresnel sky IBL, sun GGX specular). Water draws are sorted back-to-front via `SceneRenderParams::waterSortMode` (default `BackToFrontByDepth`). Opaque water (`opacity == 1`) uses depth write; alpha-blended water uses the transparent pipeline variant (depth write off).
