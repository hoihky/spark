...
`src/main.cpp` constructs `Engine` with `NewShellDemoGame()` — an interactive launcher with **21** built-in modes (3D fly scenes, maze, 2D games, scene editor prototype, material showcase, glTF PBR sample, tilemap showcase, **Dear ImGui docking demo**, and more).
