...
#include "spark/ui/core/UiSpriteSlice.hpp"
...
public:
    explicit UiPaintContext(SceneRenderParams& sceneParams) noexcept : params(&sceneParams) {}

    /** Optional UI font for layout (wrapping); set from <c>PaintUiCanvases</c> when available. */
    ...
    /** Bound per canvas in <c>PaintUiCanvases</c>. */
    void SetTheme(const UiTheme* theme) noexcept { themePtr = theme; }
    [[nodiscard]] const UiTheme& GetTheme() const noexcept;

    void SetLayoutMetrics(const UiLayoutMetrics* metrics) noexcept { metricsPtr = metrics; }
    [[nodiscard]] const UiLayoutMetrics& GetLayoutMetrics() const noexcept;
    ...
    /** Vertical gradient fill with uniform corner rounding (see <c>UiTheme::controlCornerRadius</c>). */
    ...
    /** Call once per frame before painting UI (e.g. from <c>PaintUiCanvases</c>) for sprite helpers. */
    ...
    /** Nine-slice stretch using <c>UiSpriteSlice::nineSlice</c>; falls back to <c>DrawSpriteSlice</c> when empty. */
    void DrawNineSlice(
            const UiSpriteSlice& slice,
            float x,
            float y,
