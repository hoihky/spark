...
/**
 * Semantic colors for built-in widgets. Games set a palette on `UiCanvasComponent`; each frame
 * `PaintUiCanvases` binds it to `UiPaintContext` so `IUiElement paint` reads `ctx.GetTheme()`.
 *
